...
class ELF:
    ...
    def select_parser_tools(self, elf_file, first_obj_file):
        ...
        # Check for prioritized paths
        llvm_readelf_path = "/usr/bin/llvm-readelf"
        llvm_nm_path = "/usr/bin/llvm-nm"

        # Use llvm-readelf if available at /usr/bin/llvm-readelf
        if os.path.isfile(llvm_readelf_path) and os.access(llvm_readelf_path, os.X_OK):
            ret_tool_readelf = llvm_readelf_path
        else:
        ...
        # Use llvm-nm if available at /usr/bin/llvm-nm
        if os.path.isfile(llvm_nm_path) and os.access(llvm_nm_path, os.X_OK):
            ret_tool_nm = llvm_nm_path
        else:
